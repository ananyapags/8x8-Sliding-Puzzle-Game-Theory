''' An 8-puzzle is a 3x3 grid where each tile but one contains a number from 1-8.
A tile that is next to the empty space can be moved into the empty space, leaving the position where the tile was at as the new empty space.
To solve the puzzle, you must find a valid sequence of moves such that the grid is in numerical order.
unsolved puzzle:
3 6 8 
1   4
7 2 5 
...
# run a* search ( f = g + h) to solve 8 puzzle
